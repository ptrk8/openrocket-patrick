 ...
 */
public class LookupCalculator extends AbstractAerodynamicCalculator {
	...
	public AerodynamicForces getAerodynamicForces(FlightConfiguration configuration,
		...
		BigDecimal angleOfAttack = BigDecimal.valueOf(conditions.getAOA());
		BigDecimal machNumber = BigDecimal.valueOf(conditions.getMach());

		boolean isOutsideAoA = angleOfAttack.compareTo(aeroCoefficients.getAngleOfAttackMin()) < 0 ||
								angleOfAttack.compareTo(aeroCoefficients.getAngleOfAttackMax()) > 0;
		boolean isTooSlow = machNumber.compareTo(BigDecimal.valueOf(0.1)) < 0;

		// Use original Barrowman calculator
		if (isTooSlow || isOutsideAoA) {
			// Calculate non-axial force data
